...
from rag_eval.domain.models import EvaluationResult, EvaluationRequest
...
async def evaluate_endpoint(request: EvaluationRequest, client: AsyncOpenAI = Depends(get_openai_client)):
    ...
    return await evaluation_service.evaluate(request.question, request.answer, request.context)
